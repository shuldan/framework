...
func TestReplyListener_OnResult(t *testing.T) {
	t.Parallel()
	br := newStubBroker()
	log := newRecordingLogger()
	rl := NewReplyListener(br, log)
	rl.OnResult("test.cmd", stubResultDeserializer, func(_ context.Context, _ commands.Result, _ error) error {
		return nil
	})
...
func TestReplyListener_HandleMessage_WithError(t *testing.T) {
	...
	rl := NewReplyListener(br, nil)
	var gotErr error
	rl.OnResult("test.cmd", stubResultDeserializer, func(_ context.Context, _ commands.Result, err error) error {
		gotErr = err
		return nil
	})
	errMsg := "remote error"
...
func TestReplyListener_HandleMessage_SuccessResult(t *testing.T) {
	...
	rl.OnResult("test.cmd", stubResultDeserializer, func(_ context.Context, r commands.Result, err error) error {
		gotResult = r
		return nil
	})
...
func TestReplyListener_HandleMessage_DeserializeFail(t *testing.T) {
	...
	log := newRecordingLogger()
	rl := NewReplyListener(br, log)
	rl.OnResult("test.cmd", failResultDeserializer, func(_ context.Context, _ commands.Result, _ error) error {
		return nil
	})
